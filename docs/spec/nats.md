...
shape, evolution rules. Each concern's subjects and events are defined in its
own spec document beside this one. If this document ever needs to know a
concern's details, the split has failed.
...
such as a committal change stream, is consumed whole: fold consumers subscribe
...
arrives on: the version token in the subject is the discovery, so no handshake,
...
- **A durable, causal referent**, such as a place, a working directory or content, is
...
- **An ephemeral, incidental handle**, such as a pid, a port or a reply inbox, is how
...
forwards compatible, since nobody can adhere to a future, and no many-versioned
...
any request may be answered `rejected: unsupported`, since compliance is
answering rather than implementing, and tolerance covers unknown types, fields,
and values.
...
events stay in the record, visible and monitorable, since you don't stop watching a
...
it is acting. A deployment can therefore grade them, accepting telemetry
promiscuously (even on invalid credentials) while the operational plane
demands real credentials, and that grading is only possible because the
subjects keep the planes separable.
...
publish with a reply inbox, so a stream over a requests subject becomes a second
