...
the tap-era design, with run, heartbeats and approvals evicted from the
...
- **Work is addressed to the work, never the worker.** A request that changes
  ...
  *Where it came from:* the question of what `say` addresses. Instinct said
  the bridge agent; the counter-case was Anthropic's own cluster, where every
...
- **The stream is the truth; everything else is intermediate state.** A
  ...
  *Where it came from:* two corrections. "Sending m7 to the API" is a log line
  written before the fact, and mistaking it for authority conflated telemetry
...
- **Failure is committable state, not a gap.** A turn that died is not missing
  ...
  *Where it came from:* the local analogy. On a workstation, a killed request
  leaves the conversation knowing "API request aborted", and that is the new
...
- **Control plane**: what decides how the operational plane runs, meaning
  scheduling, spawning, lifecycle and health, the behind-the-scenes machinery.
