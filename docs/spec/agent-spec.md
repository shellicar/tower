...
// agent.v1.{world}.telemetry.> — each type on its own leaf (Subjects)
export const agentTelemetry = z.discriminatedUnion('type', [
  // `host` is provenance about the world (a field, never the id). Ephemeral
  // reach-handles (pid, port, tmux coords) are not named — they ride as open
  // fields under looseObject for a deployment that wants them (nats-spec, Naming).
  z.looseObject({ type: z.literal('ready'), ts, instanceId: z.string(), host: z.string().optional() }),
  z.looseObject({ type: z.literal('pulse'), ts, instanceId: z.string(), intervalS: z.number().positive() }),
  z.looseObject({ type: z.literal('attached'), ts, instanceId: z.string(), conversationId: z.string(), cwd: z.string().optional() }),
  z.looseObject({ type: z.literal('detached'), ts, instanceId: z.string(), conversationId: z.string() }),
]);

// agent.v1.{world}.requests.> — a request whose `type` is not defined here is
// still answered: `rejected` with reason `unsupported`.
export const agentRequest = z.discriminatedUnion('type', [
  z.looseObject({ type: z.literal('service'), ts, from: sender.optional(), conversationId: z.string(), cwd: z.string().optional(), model: z.string().optional() }),
  z.looseObject({ type: z.literal('drain'), ts, from: sender.optional() }),
  z.looseObject({ type: z.literal('chdir'), ts, from: sender.optional(), conversationId: z.string(), cwd: z.string() }),
]);
