...
How implementations prove they carry the specs — `core.md`, `nats.md`,
`conversation.md`, `approval.md`, `agent.md`. This is testing, not contract:
the specs say what the wire carries; this says how an implementation proves
it. The fixture set lives in `scenarios.md`.
...
- Fixtures: the jsonl files in `fixtures/`, narrated by `scenarios.md` —
  authored with the specs, validated by first implementation contact, and
  kept alive by fix-lands-twice. Files, not prose blocks, so a copy is
  byte-diffable against the source.
