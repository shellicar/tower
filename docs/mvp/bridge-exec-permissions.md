...
1. **`builtinRules`** (`packages/claude-sdk-tools/src/Exec/builtinRules.ts`):
   ...
   accident*, because `.includes()` doesn't care where in the array `'push'` or
...
Not primarily a danger-severity call, but a **reviewability** one. A raw
...
structurally unreachable through the generic path, with no parsing needed for
...
- **Cross-platform, same config shape:** macOS (Seatbelt), Linux/WSL2+
  ...
  support glob patterns**, so a rule that works on macOS can silently behave
...
because it matches the shape already being designed for the bridge. The *engine*
...
start at all: a hard error, no results) vs **item-level** (one item within a started,
