...
(Rust/Leptos). CLAUDE.md's rule is that the two track one feature set; each open
gap here is stated precisely enough to hand to a session as a porting task on
its own. Cross items off (`[x]`) as they close; don't rewrite the doc, update it.
...
- [ ] **Inline conversation-footer approval callout diverges.** FULLY RULED (25 Jul), composed from
  ...
  `approve`/`deny` (the Svelte-wide standard, since all its chrome is lowercase: attach, cancel, say…,
...
- [ ] **Scroll-anchor "at bottom" tolerance differs.** Svelte 2px, Leptos 32px, so Leptos stays
