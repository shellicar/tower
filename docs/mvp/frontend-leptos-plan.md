...
- `Transport`: owns the socket only. `web-sys`/`gloo-net` WebSocket (or
  Leptos's own wrapper if one exists, but check rather than assume) instead of
...
2. **Shared wire types**: confirm `ws-types` drops in unchanged. Should be a
   non-finding (a repeat of Axis 2, not a new one), so call it out only if it
   isn't.
...
4. Port `Rail` second, since it's the smallest concern and the one both other
