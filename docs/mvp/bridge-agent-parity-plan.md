...
| Token refresh | missing: an expired token just fails the turn |
...
| Merge `DeleteFile`+`DeleteDirectory` into one `Delete` | per-item auto-detect file/dir (visible discriminator); non-recursive: a dir only deletes if empty, so a tree-delete means every path enumerated in leaf-first order, no hidden fan-out; ordered, per-item results, no wildcards for v1 |
...
| 2.2 Read family | 5 | `Match` | First tool dispatching on two stream types (`File[]` path-match vs `Line[]` content-match), so it needs `Read` to exist to produce a content stream to test against. | none |
...
| 2.3 Binary read | 8 | `ReadFile` | MIME-sniffed, PDF/image → attachment block (`objects.rs`). Its own standalone tool, not part of the pipe family, and separate from composable `Read`. | none |
...
| 2.4 Paging | 10 | Auto-invoke over large tool outputs | The "walk and replace what's too big" wiring, depending on 9 existing, and on 2.1/2.2 being the tools that'll actually trip the size threshold. | none |
