...
`rejected` with reason `already_settled`, information rather than an error. The
...
settlement rides, so every connected session drops it from the pending count
together.
...
absent means untitled, so show the id. The `list` is the only carrier: `row`
...
conversation**. A conversation has at most one, with no candidate set, nothing
...
the streaming text that preceded it is superseded, so clear any streaming
...
`telemetry.usage` into per-conversation totals, where the token counts are
...
while open, the same gating as `message`, because this is per-conversation
...
like any activity, and that is awareness and is not gated; this frame is the
content.)
...
- `hint` says what the bytes are, whether a media type (`image/png`,
...
GET /ref/{id}  + Range → a slice, previewing the first 4 KB of a 500 KB result
```
...
The wire's evolution rules, both directions: producers only add, meaning new
message types and new optional fields; consumers tolerate, meaning unknown `type` skipped
