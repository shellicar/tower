...
- **The layer** (L3/L4): how bytes move, via fds, pipes, sockets, TCP.
- **Colloquial**: sometimes used for a whole bridge ("the NATS transport"). The precise word is **bridge**.

## orchestration

- **Control-plane orchestration** (Kubernetes sense): orchestrating *resources*, meaning spawn, schedule, lifecycle, health. Tower's job.
- **Workflow orchestration** (fleet sense): orchestrating *the work*, meaning which role does what, routing on verdicts. The orchestration logic.
