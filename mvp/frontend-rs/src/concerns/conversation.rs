...
//! frontend-architecture.md), mirroring the control's conversation.svelte.ts. It
//! owns a keyed map of open conversations and their content, folds its OWN
//! slices of the wire (its convs' messages, streaming, query closures), and
//! drives say/cancel.
//!
//! Correlation is local, the fan-out way: an outbound say/cancel mints a request
//! id, the concern records it, and the matching `say_result`/`cancel_result`
//! frame is resolved in `apply` by that id — no promise, no await; the result
//! arrives through the same drain as every other frame. Action methods return
//! the `ClientMsg` to send; the app (which owns the transport and the id mint)
//! sends it. So this concern touches no socket and, like every concern, its
//! `apply(&mut self, &ServerMsg)` borrows only itself.
...
mod tests {
    ...
    fn the_committed_say_supersedes_the_pending_one() {
        ...
        // The committed user message for that query clears the pending say.
        c.apply(&ServerMsg::Message {
            conv: "a".into(),
