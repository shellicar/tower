...
//! locally, the fan-out way), and the settlement arrives as an `approval` frame
//! like any other.
//!
//! This is the "shared value" done by events, not a shared store — Decision 2's
//! hard case. The rail folds its own marker, the panel its own, this view the
//! whole list; no approval surface is shared. The conversation LABEL for an ask
//! is read from the rail concern by the renderer, not folded here.
