...
- A `retry` control line makes bridge retry a model request that fails before its stream starts, so one transient failure no longer ends a conversation's query.
